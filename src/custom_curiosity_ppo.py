...
from stable_baselines3.common.utils import explained_variance, get_linear_fn, constant_fn, update_learning_rate
...
class CustomPPO(PPO):
    ...
    def _setup_model(self) -> None:
        self._setup_lr_schedule(self.learning_rate)
        self._setup_clip_schedule(self.clip_range, self.clip_range_vf)
    ...
    def _get_schedule_fn_clip(self, value_schedule: Union[Callable, float]) -> Callable:
        """
        ...
        if isinstance(value_schedule, (float, int)):
            ...
            # value_schedule = get_linear_fn(0.25, 0.15, 0.5)
        else:
            assert callable(value_schedule)
        return value_schedule

    def _get_schedule_fn_lr(self, value_schedule: Union[Callable, float]) -> Callable:
        """
        Transform (if needed) learning rate and clip range (for PPO)
        to callable.
    ...
    def _setup_lr_schedule(self, learning_rate) -> None:
        """Transform to callable if needed."""
        self.lr_schedule = self._get_schedule_fn_lr(learning_rate)

    def _setup_clip_schedule(self, clip_range, clip_range_vf):
        # Initialize schedules for policy/value clipping
        self.clip_range = self._get_schedule_fn_clip(clip_range)
        if clip_range_vf is not None:
            ...
            self.clip_range_vf = self._get_schedule_fn_clip(self.clip_range_vf)
    ...
    def _update_learning_rate(self, optimizers: Union[List[th.optim.Optimizer], th.optim.Optimizer]) -> None:
        ...
        # logger.record("train/learning_rate", min(3e-4, self.rm_intrinsic_rewards.mean().item()))

        if not isinstance(optimizers, list):
            optimizers = [optimizers]
        for optimizer in optimizers:
            # update_learning_rate(optimizer, self.lr_schedule(self._current_progress_remaining))
            update_learning_rate(optimizer, min(3e-4, self.rm_intrinsic_rewards.mean().item()))
